...
public class DatabaseMovieManager implements MovieManager{
	...
	public static synchronized MovieManager getInstance() {
		if (instance == null) {
			instance = new DatabaseMovieManager();
		}
		return instance;
	}
	
	private DatabaseMovieManager() {
		this.connectionManager = new ConnectorManager("org.postgresql.Driver", USERNAME, PASSWORD);
		this.factory = DatabaseObjectFactory.getInstance();
	}
	...
	public Movie getMovie(int id) {
		...
		try {
			...
			if (results.next()) {
				...
				newMovie = factory.getNewMovie(movieId, title, director, imgUrl, duration, release, synopsis, creation);
			}
			connection.commit();
			connection.close();
	...
	private Iterable<Movie> getMoviesGeneric(String criteria) {
		...
		try {
			...
			while(results.next()) {
				...
				Movie item = factory.getNewMovie(id, title, director, imgUrl, duration, release, synopsis, creation);
	...
	public Set<Genre> GetMovieGenres(Movie movie) {
		...
		try {
			...
			while (results.next()) {
				set.add(factory.getNewGenre(results.getString(1)));
			}
	...
	public Iterable<Movie> searchWith(String q){
		...
		else
		{
			...
			while(result.next()) {
				...
				Movie item = factory.getNewMovie(id, title, director, "", duration, release, "");
				ret.add(item);
			}
