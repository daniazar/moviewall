...
public class DatabaseCommentManager implements CommentManager {
	...
	private static DatabaseCommentManager instance;
	
	private ConnectorManager connectionManager;
	private DatabaseObjectFactory objFact;

	public static synchronized DatabaseCommentManager getInstance() {
		if (instance == null) {
			instance = new DatabaseCommentManager();
		}
		return instance;
	}
	
	private DatabaseCommentManager() {
	...
	public Iterable<Comment> getAll() {
		...
		try {
			...
			while (results.next()) {
				...
				list.add( objFact.getNewComment(results.getInt(1), results.getString(2), results.getString(3), date2,results.getInt(4), results.getInt(6)));
			}
			connection.close();
		} catch (SQLException e) {
			throw new DatabaseException(e.getMessage(), e);
	...
	public Comment getComment(String id) {
		...
		try {
			...
			while (results.next()) {
				...
				c= ( objFact.getNewComment(results.getInt(1), results.getString(2), results.getString(3), date2,results.getInt(4), results.getInt(6)));
			}
			connection.close();
		} catch (SQLException e) {
			throw new DatabaseException(e.getMessage(), e);
	...
	public void saveComment(Comment comment) {
		...
		commentStmt.setString(2, comment.getUserId());
		commentStmt.setInt(3, comment.getMovieId());
